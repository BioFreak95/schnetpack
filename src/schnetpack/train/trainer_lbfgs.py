...
class Trainer_LBFGS:
   ...
   """
        ...
        try:
            for _ in range(n_epochs):
                ...
                for train_batch in train_iter:
                    ...
                    # LBFGS is very expensive -> So give a print to track the speed
                    print('Step ', num, ' of ', len(train_iter))

                    train_batch = {k: v.to(device) for k, v in train_batch.items()}
                    self.act_batch = train_batch

                    loss, result = self.optimizer.step(self.closure)
                    self.step += 1
    ...
    def closure(self, res=False):
        self.optimizer.zero_grad()

        for h in self.hooks:
            h.on_batch_begin(self, self.act_batch)

        result = self._model(self.act_batch)
        loss = self.loss_fn(self.act_batch, result)
        loss.backward()
        if res:
            return loss, result
        return loss
